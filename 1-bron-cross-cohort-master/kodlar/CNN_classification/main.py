...
import compute_CI as compute_CI
...
def main(argv):
    ...
    if not config.train_only:
        ...
        plot_ROC(results["test"]["tpr"], results["test"]["fpr"], results["test"]["auc"], config.output_dir)
    ...
    if not config.train_only:
        ...
        print('mean Acc:', '%.3f' % test_results['acc_mean'], ' (',  '%.3f' % test_results['acc_95ci'][0], ' - ', '%.3f' % test_results['acc_95ci'][1], ')')
        print('mean AUC:', '%.3f' % test_results['auc_mean'], ' (',  '%.3f' % test_results['auc_95ci'][0], ' - ', '%.3f' % test_results['auc_95ci'][1], ')') 
