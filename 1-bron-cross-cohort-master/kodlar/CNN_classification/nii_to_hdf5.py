...
def main():
    ...
    # create output dir
    if WB:
        save_path_WB = f"/media/data/ebron/data_{dataset}_{datatype}_WB/"
        create_data_directory(save_path_WB)
    if downsampling:
        save_path_f4 = f"bigr_mount/ebron/ADNI/data_{dataset}_{datatype}_f4/"
        create_data_directory(save_path_f4)

    # set path to data
    if dataset == "ADNI":
        if datatype == "GM":
            data_path = "cartesius_mount/ADNI/Template_space/*_bl/Brain_image_in_template_space/gmModulatedJacobian.nii.gz"
        elif datatype == "T1":
            data_path = "cartesius_mount/ADNI/Template_space/*_bl/Brain_image_in_MNI_space/result.nii.gz"
        elif datatype == "T1m":
            data_path = "cartesius_mount/ADNI/Template_space/*_bl/Brain_image_in_template_space/T1wModulatedJacobian.nii.gz"
    elif dataset == "PSI":
        if datatype == "GM":
            data_path = "cartesius_mount/Parelsnoer/Template_space/PSI*/Brain_image_in_template_space/gmModulatedJacobian.nii.gz"
        elif datatype == "T1":
            data_path = "cartesius_mount/Parelsnoer/Template_space/PSI*/Brain_image_in_MNI_space/result.nii.gz"
        elif datatype == "T1m":
            data_path = "cartesius_mount/Parelsnoer/Template_space/PSI*/Brain_image_in_template_space/T1wModulatedJacobian.nii.gz"

    # create mask of brain
    template_file = "cartesius_mount/ADNI/Template_space/brain_mask_in_template_space.nii.gz"
...
#            np.save(save_path_f4 + subject + '.npy', downsampled)
        ...
        if WB:
            ...
            if not os.path.exists(save_path_WB + subject + ".h5py"):
                print (subject)
